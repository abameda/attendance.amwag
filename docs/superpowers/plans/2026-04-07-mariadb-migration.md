...
export const attendance = mysqlTable(
  ...
  (t) => ({
    ...
    dashIdx: index('idx_attendance_user_date_status').on(t.userId, t.date, t.status),
  })
);
...
- [ ] **Step 2: Add `global_settings` seed row**
...
- [ ] **Step 5: Run migration against local MariaDB**
...
export async function middleware(request: NextRequest) {
  ...
  const isPublicRoute = publicRoutes.some((r) => pathWithoutLocale.startsWith(r));
...
const { data: profile } = await supabase.from('profiles').select('shift_start, shift_end').eq('id', user.id).single();
...
- [ ] **Step 1: Rewrite `check-in/route.ts`**
...
- [ ] **Step 2: Rewrite `check-out/route.ts`**
...
- [ ] **Step 3: Rewrite `attendance/route.ts` (GET)**
...
- [ ] **Step 4: Rewrite `attendance/summary/route.ts`**
...
const rows = await db
  .select({
    ...
    totalOvertime: sql<number>`SUM(${attendance.overtimeMinutes})`.mapWith(Number),
    presentCount: sql<number>`SUM(CASE WHEN ${attendance.status} = 'present' THEN 1 ELSE 0 END)`.mapWith(Number),
    // ...
  })
  .from(attendance)
  .where(/* date range */)
...
- [ ] **Step 6: Commit** (one commit for all four files)
...
- [ ] **Step 1: Overwrite `employees/route.ts`**
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: { id, email: normalizedEmail } });
  } catch (err) {
...
- [ ] **Step 3: Rewrite `bulk-import/route.ts`**
...
- [ ] **Step 4: Create admin reset-password endpoint**
...
- [ ] **Step 1: Read existing finalization lib**
...
- [ ] **Step 2: Translate function by function**
...
export async function closeMissingCheckouts() {
  ...
  return { closed: (result as unknown as { affectedRows?: number }).affectedRows ?? 0 };
}
```
...
- [ ] **Step 3: Rewrite the finalize endpoint**
...
- [ ] **Step 1: Inspect the old mark-absent route**
...
- [ ] **Step 2: Create new internal cron endpoint**
...
- [ ] **Step 3: Delete or rewrite the old route**
...
- [ ] **Step 4: `admin/employees/page.tsx`**
...
- [ ] **Step 5: Create `GET /api/auth/me` endpoint**
...
- [ ] **Step 6: Create `GET /api/attendance/me` for employee history**
...
- [ ] **Step 1: Verify nothing still imports Supabase**
...
- [ ] **Step 11: Cron unauthorized check**
