...
test('attendance PDF drawing uses branded cards, blue table headers, and status badges', () => {
  ...
  assert.ok(
    calls.some((call) => call.method === 'setFillColor' && call.args.join(',') === '41,160,211'),
    'primary blue should be used as a fill color'
  );
  assert.ok(
    calls.some((call) => call.method === 'rect' && call.args[0] === 0 && call.args[1] === 0 && call.args[3] === 5),
    'header should draw a 5mm top blue border'
  );
  assert.ok(
    calls.some((call) => call.method === 'setFillColor' && call.args.join(',') === '251,176,59'),
    'secondary gold should be used for summary card accents'
  );
  assert.ok(
  ...
  assert.ok(
    calls.some((call) => call.method === 'setFillColor' && call.args.join(',') === '220,252,231'),
    'present status should draw a light green badge background'
  );
  assert.ok(
    calls.some((call) => call.method === 'setFillColor' && call.args.join(',') === '254,226,226'),
    'absent status should draw a light red badge background'
  );
});
